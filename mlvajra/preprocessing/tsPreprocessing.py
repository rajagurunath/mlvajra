"""
shamelessly copied from Fastai repository with few changes 
...
class TabularProc():
    ...
    def __init__(self,cat_names,cont_names):
        self.cat_names:List[str]=cat_names
        self.cont_names:List[str]=cont_names
...
class FillMissing(TabularProc):
    ...
    def __init__(self,cat_names,cont_names,fill_strategy:FillStrategy=FillStrategy.MEDIAN,
                    add_col_bool:bool=True,fill_Val:float=0):
        super(FillMissing, self).__init__(cat_names,cont_names)
        self.fill_strategy=fill_strategy
        self.add_col=add_col_bool
    ...
    def apply_test(self, df:DataFrame):
        ...
        for name in self.cont_names:
            if name in self.na_dict:
                # only add columns if the columns are present while applying Training
                # if name+'_na' in self.cat_names:
                #     df[name+'_na'] = pd.isnull(df[name])
                if self.add_col:
                    df[name+'_na'] = pd.isnull(df[name])
                    if name+'_na' not in self.cat_names: self.cat_names.append(name+'_na')
...
if __name__=='__main__':
    ...
    df_main=pdr.get_data_fred('GS10')
    df_main=df_main.reset_index()
    df=df_main
    print(df.dtypes,df.head())
    make_date(df,'DATE')
    print(df.dtypes,df.head())
    df1=add_datepart(df,field_name='DATE',drop=False)
    print(df1.dtypes)
    df2=add_cyclic_datepart(df_main.copy(),'DATE')
    df1['uni_index']=df1.index
    df3=add_elapsed_times(df1,'GS10','DATE','uni_index')
    print(df3.dtypes,df3.head())
    df3['GS10'][100]=np.nan
    fm=FillMissing(['DATEIs_quarter_end'],cont_names=['GS10'])
    df4=fm.apply_train(df3)
